...
> 本项目基于源项目 [Wangnov/gpt-image-2-skill](https://github.com/Wangnov/gpt-image-2-skill) 修改而来，感谢原作者提供了完整、扎实且高质量的开源基础。
>
> 这次调整的初衷，是为了更好支持部分中转站返回 `image-2` 时的一些特殊情况，并进一步增强这个 skill 的独立性。围绕这个目标，项目已经完全使用 TypeScript 重新实现原先 Rust 模块所承担的能力，便于在当前 skill 体系内独立运行与维护。
...
也可以从 [GitHub Releases](https://github.com/emengweb/gpt-image-2-skill/releases/latest) 下载对应平台安装包：
...
```bash
git clone https://github.com/emengweb/gpt-image-2-skill /tmp/gpt-image-2-skill
...
> This repository is adapted from [Wangnov/gpt-image-2-skill](https://github.com/Wangnov/gpt-image-2-skill). Many thanks to the original author for building and open-sourcing such a solid foundation.
>
> The visible motivation for this fork is to better support some special `image-2` response patterns returned by certain relay services, while also making the skill more self-contained. To support that goal, the capabilities previously handled by the Rust module have been fully reimplemented in TypeScript for more independent operation and maintenance inside this skill.
...
You can also download the right installer from [GitHub Releases](https://github.com/emengweb/gpt-image-2-skill/releases/latest):
